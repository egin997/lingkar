-- Stable fingerprint for all repository-owned schema objects through Phase 3.
...
object_definitions as (
  select format(
    'column:%I.%I.%I:%s:%s:%s', namespace.nspname, relation.relname, attribute.attname,
  ...
  select format('constraint:%I.%I:%s', namespace.nspname, relation.relname, pg_get_constraintdef(constraint_row.oid, true))
  ...
  select format(
    'policy:%I.%I:%s:%s:%s:%s:%s', schemaname, tablename, policyname, cmd, roles,
    coalesce(qual, ''), coalesce(with_check, '')
  )
  from pg_catalog.pg_policies
  where (schemaname = 'public' and tablename in (select table_name from repository_tables))
     or (schemaname = 'storage' and tablename = 'objects' and policyname like 'content_media_objects_%')
  ...
  where (namespace.nspname = 'private' and procedure.proname in (
      ...
      'is_space_moderator', 'apply_space_reputation_entry', 'can_view_content_post',
      'can_view_content_reply', 'capture_content_post_revision',
      'capture_content_reply_revision', 'begin_content_mutation',
      'complete_content_mutation', 'assert_content_actor', 'replace_content_mentions'
    ))
  ...
  select format(
    'bucket:%s:%s:%s:%s', bucket.id, bucket.public, bucket.file_size_limit,
    array_to_string(bucket.allowed_mime_types, ',')
  )
  from storage.buckets bucket where bucket.id = 'content-media'
)
select
  md5(string_agg(definition, E'\n' order by definition)) as phase_3_schema_fingerprint,
